...
class UserValidation(object):
    ...
    def valid_email(self, email=None):
        ...
        if user_qs.exists():
            raise ValidationError('usedemail')
        elif re.search(r"^[\w\.\+\-]+\@[\w]+\.[a-z]{2,3}$", email) is None:
            raise ValidationError(' wrong email_format')
        return True
    ...
    def valid_username(self, username=None):
        ...
        if user_qs.exists():
            raise ValidationError('usedname')
        elif len(username) < 3:
            raise ValidationError('shortname')
    ...
    def valid_password(self, password=None):
        ...
        if len(password) < 8:
            raise ValidationError('short_pwd')
    ...
    def valid_login_email(self, email=None):
        ...
        if re.search(r"^[\w\.\+\-]+\@[\w]+\.[a-z]{2,3}$", email) is None:
            raise ValidationError('email_format')
        elif not user_qs.exists():
            raise ValidationError('unregistered_email')
        return True
